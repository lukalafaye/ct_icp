...
namespace ct_icp {
    ...
    // A Cost Functor for a standard Point-to-Plane
    struct PointToPlaneFunctor {
        ...
        bool operator()(const T *const rot_params, const T *const trans_params, T *residual) const {
            ...
            transformed(0) += trans_params[0];
            transformed(1) += trans_params[1];
            transformed(2) += trans_params[2];

            // Use subtraction and an explicit dot product to obtain a scalar.
            residual[0] = weight_ *
                          ((reference_.template cast<T>() - transformed)
                           .dot(reference_normal_.template cast<T>()));
            return true;
        }
    ...
    struct CTPointToPlaneFunctor {
        ...
        bool operator()(const T *const begin_rot_params, const T *begin_trans_params,
            ...
            transformed(0) += alpha_m * begin_trans_params[0] + alpha_timestamps_ * end_trans_params[0];
            transformed(1) += alpha_m * begin_trans_params[1] + alpha_timestamps_ * end_trans_params[1];
            transformed(2) += alpha_m * begin_trans_params[2] + alpha_timestamps_ * end_trans_params[2];

            // Use subtraction and dot() to compute the residual scalar.
            residual[0] = weight_ *
                          ((reference_point_.template cast<T>() - transformed)
                           .dot(reference_normal_.template cast<T>()));
            return true;
        }
    ...
    struct LocationConsistencyFunctor {
        ...
        bool operator()(const T *const location_params, T *residual) const {
            residual[0] = beta_ * (location_params[0] - previous_location_(0));
            residual[1] = beta_ * (location_params[1] - previous_location_(1));
            residual[2] = beta_ * (location_params[2] - previous_location_(2));
            return true;
        }
    ...
    struct OrientationConsistencyFunctor {
        ...
        OrientationConsistencyFunctor(const Eigen::Quaterniond &previous_orientation,
                                      double beta)
            : beta_(beta), previous_orientation_(previous_orientation) {}
    ...
    struct ConstantVelocityFunctor {
        ...
        bool operator()(const T *const begin_t, const T *const end_t, T *residual) const {
            residual[0] = beta_ * (end_t[0] - begin_t[0] - previous_velocity_(0));
            residual[1] = beta_ * (end_t[1] - begin_t[1] - previous_velocity_(1));
            residual[2] = beta_ * (end_t[2] - begin_t[2] - previous_velocity_(2));
            return true;
        }
